...
# ── Chart: Donut importance ───────────────────────────────────────────────────
def make_donut(mdl):
    rf  = mdl.named_steps["clf"]
    imp = rf.feature_importances_
    short = ["RMS","Max ET-HT","Min ET-HT","Std Dev ET-HT",
             "Mean Response Time","Max Hold Time","Min Hold Time","Std Hold Time","Direction Changes"]
    ...
    wedges, _, autotexts = ax.pie(
        imp[idx], labels=None, autopct="%1.0f%%", startangle=90,
    ...
    ax.legend(wedges, [short[i] for i in idx],
              loc="lower center", bbox_to_anchor=(0.5, -0.12),
              ncol=3, fontsize=7.5, framealpha=0, labelcolor="#7aaac8")
    ax.set_title("Feature Importance", fontsize=10, color="#7aaac8", pad=10)
    fig.tight_layout(pad=0.5)
    return fig
...
with right:
    ...
    if prob is not None:
        ...
        with c4:
            st.markdown("<div class='neo-card' style='padding:1rem;'>", unsafe_allow_html=True)
            st.pyplot(make_donut(model), use_container_width=True)
            st.markdown("</div>", unsafe_allow_html=True)
        ...
        <div style='background:linear-gradient(135deg,#0d1f35,#0a1826);
                    ...
                    padding:4rem 2rem; text-align:center; margin-top:0.2rem;'>
          ...
          <div style='font-size:0.7rem; color:#1a3a5c;
            ...
            Gauge · Radar · Feature Bars · Importance Donut will appear here
          </div>
        </div>
        """, unsafe_allow_html=True)
        st.markdown("<div style='height:0.8rem'></div>", unsafe_allow_html=True)
