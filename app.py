...
def load_model():
    ...
    # Support both old format (pipeline only) and new format (dict with metadata)
...
def make_gauge(prob):
    ...
    ax.text(0.5, 0.76, "PARKINSON'S PROBABILITY", ha="center", va="center",
            transform=ax.transAxes, fontsize=7, color="#5a8aaa")
...
# ────────────────────────────────────────────────────────────────────────────
#  STATS STRIP  — session + model metadata
# ────────────────────────────────────────────────────────────────────────────
...
# Pull model metadata if available
train_acc = model_meta.get("train_accuracy", None)
train_auc = model_meta.get("train_auc", None)
n_pd      = model_meta.get("n_pd", "—")
n_healthy = model_meta.get("n_healthy", "—")
cv_auc    = model_meta.get("cv_auc_mean", None)

# Row 1 – session stats
s1, s2, s3, s4 = st.columns(4)
for col, val, lbl in [
    (s1, str(total),       "ANALYSES RUN"),
    (s2, f"{avg_p:.1f}%",  "AVG PD SCORE"),
    (s3, str(high_r),      "HIGH-RISK COUNT"),
    (s4, "RF · 300 TREES", "MODEL TYPE"),
]:
    col.markdown(f"""
...
# Row 2 – model performance stats (only if metadata present)
if train_acc is not None:
    m1, m2, m3, m4 = st.columns(4)
    for col, val, lbl in [
        (m1, f"{train_acc*100:.1f}%",                    "MODEL ACCURACY"),
        (m2, f"{train_auc:.3f}" if train_auc else "—",   "ROC-AUC SCORE"),
        (m3, f"{cv_auc:.3f}" if cv_auc else "—",         "5-FOLD CV AUC"),
        (m4, f"{n_pd} PD / {n_healthy} Healthy",         "TRAINING DATA"),
    ]:
        col.markdown(f"""
        <div class='metric-box'>
    ...
    if st.session_state.last_feats:
        with st.expander("🔬  Raw Feature Values"):
            fdf = pd.DataFrame({
                ...
                "Value": [round(st.session_state.last_feats[c], 4)
